...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let lastDistance = null;  // Store last pinch distance

    // Event listener for zoom (Desktop)
    mainCanvas.addEventListener("wheel", (e) => {
        e.preventDefault();
        // Zoom factor
        let zoomFactor = e.deltaY > 0 ? 0.8 : 1.2;
        handleZoom(e.clientX, e.clientY, zoomFactor);
    });

    // Event listeners for pan (Desktop)
    mainCanvas.addEventListener("mousedown", (e) => {
        isDragging = true;
        lastX = e.clientX;
        lastY = e.clientY;

        // Rasterizer optimization - canvas width and height are reduced for smoother pan/zoom
    ...
    mainCanvas.addEventListener("mousemove", (e) => {
        if (isDragging) handlePan(e.clientX, e.clientY);
    });
    mainCanvas.addEventListener("mouseup", () => stopDragging());


    // Event listeners Pinch-to-Zoom (Mobile) and Drag-to-Pan (Mobile)
    ...
    mainCanvas.addEventListener("touchmove", (e) => {
        ...
        if (e.touches.length === 2) {
            ...
            if (lastDistance) {
                let zoomFactor = newDistance > lastDistance ? 1.02:0.98;  // Use much finer values compared to Desktop/mouse for reduced zoom sensitivity
    ...
    mainCanvas.addEventListener("touchend", (e) => {
        if (e.touches.length === 0) stopDragging();
    });
    ...
    // Helper functions
    function handleZoom(clientX, clientY, zoomFactor) {
    ...
    function handlePan(clientX, clientY) {
        let aspectRatio = mainCanvas.width / mainCanvas.height;
        let dx = (clientX - lastX) / mainCanvas.width * (4.0 / zoomMain) * aspectRatio * dpr / 2;
        let dy = (lastY - clientY) / mainCanvas.height * (4.0 / zoomMain) * dpr / 2;
